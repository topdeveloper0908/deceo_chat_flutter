...
app.get('/', async (req, res, next) => {
  if ( req.apiGateway.event.triggerSource !== 'PostConfirmation_ConfirmSignUp' || !req.apiGateway.event.request.userAttributes.email || !req.apiGateway.event.request.userAttributes["custom:groupname"] ) {
    const err = new Error('username and groupname are required');
    err.statusCode = 400;
    return next(err);
  }
  ...
  try {
      const response = await addUserToGroup(req.apiGateway.event.request.userAttributes.email, req.apiGateway.event.request.userAttributes["custom:groupname"]);
      console.log("res", req.apiGateway.event);
      res.status(200).send(req.apiGateway.event);
  } catch (err) {
    next(err);
  }
});
